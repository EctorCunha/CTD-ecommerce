    ...
    return (
        <header>
            ...
            <div className='icons-header'>
                ...
                <img width="40" height="40" viewBox="0 0 40 40" className='icon_adm open-modal' onClick={openModal} src={icon_adm} alt="icon_adm" />
                <dialog className="modal">
                    <h2 className='modal-title'>ÁREA RESTRITA</h2>
                    <form className="login-form" method="dialog">
                        <input type="email" name="email" id="email" placeholder="EMAIL" required />
                        <input type="password" name="password" id="password" placeholder="SENHA" required />
                        <Link to={'/adm'} ><button className="form-button" onClick={closeModal}>ENTRAR</button></Link>
                    </form>
                </dialog>
            </div>
        </header>
